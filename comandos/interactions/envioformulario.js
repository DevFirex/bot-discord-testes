...
module.exports = {
  async execute(interaction, cache, client) {
    try {
      let cacheKey = interaction.message
      ...
      let viewMode =
        interaction.customId === "enter_link_delete_mode" ||
        (interaction.customId.startsWith("delete_link_index_") &&
          userData.linkButtons.length > 0)
          ? "delete_links"
          : "editor";
      ...
      if (interaction.isModalSubmit()) {
        ...
        if (interaction.customId === "modal_add_link_button") {
          ...
          let targetWebhook = webhooks.find(
            (wh) =>
              wh.ownerId === client.user.id && wh.name === "embed-bot-publisher"
          );
          if (!targetWebhook)
            targetWebhook = await interaction.channel.createWebhook({
              name: "embed-bot-publisher",
            });
          ...
          await webhookClient.send({
            ...
            avatarURL:
              cleanUrl(val("webhook_icon_url")) ||
              client.user.displayAvatarURL(),
            embeds:
              activeEmbedIndex === -1
                ? userData.embeds.map((e) => EmbedBuilder.from(e))
                : [currentEmbed],
          ...
          if (interaction.customId === "modal_update_title")
            currentEmbed
              .setTitle(val("title_input"))
              .setURL(cleanUrl(val("url_input")));
            ...
            currentEmbed
              .setImage(cleanUrl(val("image_url")))
              .setThumbnail(cleanUrl(val("thumbnail_url")));
          else if (interaction.customId === "modal_update_footer")
            currentEmbed
              .setFooter({
                text: val("footer_text"),
      ...
      if (interaction.isButton()) {
        ...
        if (interaction.customId.startsWith("delete_link_index_")) {
          const idx = parseInt(
            interaction.customId.split("__")[0].split("_").pop()
          );
      ...
      row1.components[0].options.forEach(
        (opt) => (opt.default = opt.value === String(activeEmbedIndex))
      );

      if (viewMode === "delete_links" && userData.linkButtons.length > 0) {
        const deleteRows = ui.createDeleteModeRows(
          userData.linkButtons,
          cacheKey
        );
        await interaction.update({
          content: `## 🗑️ Modo de Exclusão\n${successMessage}`,
          embeds: [currentEmbed],
          components: [row1, ...deleteRows],
        });
      } else if (activeEmbedIndex !== -1 && currentEmbed) {
        userData.embeds[activeEmbedIndex] = currentEmbed.toJSON();
        let allButtons = userData.editingButtonsJSON
          .map((b) => ButtonBuilder.from(b))
          .filter(
            (btn) =>
              ![
                "add_link_button",
                "enter_link_delete_mode",
                "delete_embed",
              ].includes(btn.data.custom_id)
          );

        ui.createLinkManagementButtons(userData.linkButtons).forEach(
          (config) => {
            allButtons.push(
              new ButtonBuilder()
        ...
        if (activeEmbedIndex > 0)
          allButtons.push(
            new ButtonBuilder()
              ...
              .setLabel(`🗑️ Deletar Embed ${activeEmbedIndex + 1}`)
